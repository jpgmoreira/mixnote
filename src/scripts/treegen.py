...
class Node:
    def __init__(self, node_id, node_type, text, parent_id=None):
        ...
        # Dir specific
        self.open = True if node_type == "dir" else None
        ...
        self.nFileDesc = 0 if node_type == "dir" else None

        # File specific
        self.noteId = random_id() if node_type == "file" else None
...
def create_random_tree(total_nodes: int):
    ...
    def add_child(parent: Optional[Node]):
        nonlocal total_nodes, position_counter
        if total_nodes <= 0:
            return None
        ...
        if parent:
            ...
            if sub["headId"] is None:
                sub["headId"] = node.id
                sub["tailId"] = node.id
                node.ui["isLastChild"] = True
            else:
                tail = id_to_node[sub["tailId"]]
                node.prevId = tail["id"]
                tail["nextId"] = node.id
                tail["ui"]["isLastChild"] = False
                sub["tailId"] = node.id
                node.ui["isLastChild"] = True
    ...
    # Start by creating nodes at the root
    roots = []
